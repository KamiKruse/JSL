...
function summerJamCount(arr){
    ...
    arr.forEach(function(val){
        ...
        {
            // console.log(`${val.name}, ${val.year}, month: ${val.month}`);
           count++
        }
    })
    return count
...
{
    return arr.map(function(val){
        return (parseInt(val.duration[0]*60) + parseInt(val.duration[2]) + parseInt(val.duration[3]))
    })
}
...
{
    return arr.filter(function(songs){
...
{
    return arr.filter(function(songs){
        return parseInt(songs.duration[0]) < 3 
    })
}
...
{
  return  arr.filter(function(songs){
        return songs.artist === str
    })
}

// console.log(getSongsByArtist(songs, 'The Weeknd'));

/// Using Reduce

function summerJamCountReduce(arr)
{
...
{
    return arr.reduce(function(acc, next){
       acc += next.weeksAtNumberOne/arr.length
       return acc
    },0)
}
